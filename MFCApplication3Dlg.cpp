...
void CMFCApplication3Dlg::OnBnClickedButtonSend()
{
	...
	GetDlgItemText(IDC_EDIT2_CHAT_TEXT, msg); // Edit Control에 쓴 내용은 가져온다.
	if (!msg.IsEmpty()) // 가져온 내용이 있는지 확인.
	{
		//m_List.AddString(msg); // 내용이 있다면 해당 문자를 List Box에서 출력.
		SetDlgItemText(IDC_EDIT2_CHAT_TEXT, _T(""));// Edit Control에 적혀있는 내용을 지운다.
		...
		temp = msg.Left(1);
		recv_packet.cData1 = *(temp.GetBuffer());
		
		temp = msg.Mid(1,1);
		recv_packet.cData2 = *(temp.GetBuffer());
		
		temp = msg.Mid(2,1);
		recv_packet.iData = _ttoi(temp);
		// m_Client.Send() 함수를 이용하여 데이터를 전송할 수 있다.
		m_Client.Send((char*)&recv_packet, sizeof(recv_packet));
	}
}
